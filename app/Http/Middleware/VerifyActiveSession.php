...
class VerifyActiveSession
{
	...
	public function handle(Request $request, Closure $next): mixed {
		...
		if ($originalIp != $currentIp && $originalUserAgent != $currentUserAgent) {
			...
			// For high security scenarios, you might want to terminate the session
			// Uncomment the following code to enable this behavior
			/*
			$currentSession->update([
				'is_active' => false,
				'logged_out_at' => Carbon::now(),
			...
			Session::flush();
			return redirect()->route('login')
				->with('error', 'Your session has been terminated due to security concerns. Please log in again.');
			*/
		}
