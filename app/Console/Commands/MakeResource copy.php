...
//    php artisan make:resource Country --path=Admin --table=countries

	protected $signature = 'make:resource {name : The name of the resource (e.g., Country)}
                                         {--path=Admin : The path for the controller (e.g., Admin)}
                                         {--table= : The database table name (defaults to lowercase plural of the resource name)}';
	 ...
	 * @var string
	 */
	protected $description = 'Create controller, repository interface, implementation, request files, and register in service provider';
	...
	public function handle()
	{
		...
		$table = $this->option('table') ?: Str::plural(Str::snake($name));

		$this->info("Creating resource files for: {$name}");

		// Create the controller
		$this->createController($name, $path);

		// Create request files
		$this->createRequestFiles($name, $table);
	...
	protected function createController(string $name, string $path): void
	{
		...
		$pluralName = Str::plural($lowercaseName);
...
class {$name}Controller extends Controller
{
   ...
   public function index(Request \$request): Response {
       \${$pluralName} = \$this->{$lowercaseName}Repository->paginate(\$request);
    ...
    public function store({$name}StoreRequest \$request): RedirectResponse {
       try {
          ...
          return success_route('{$lowercaseName}.index', '{$name} created successfully.');
       } catch (\Exception \$e) {
          return error_route('{$lowercaseName}.index', 'Failed to create {$lowercaseName}: ' . \$e->getMessage());
       }
    }
    ...
    public function update({$name}UpdateRequest \$request, int \$id): RedirectResponse {
       try {
          ...
          return success_route('{$lowercaseName}.index', '{$name} updated successfully.');

       } catch (\Exception \$e) {
          return error_route('{$lowercaseName}.index', 'Failed to update {$lowercaseName}: ' . \$e->getMessage());
       }
    }
    ...
    public function destroy(int \$id): RedirectResponse {
       try {
          ...
          return success_route('{$lowercaseName}.index', '{$name} deleted successfully.');
       } catch (\Exception \$e) {
          return error_route('{$lowercaseName}.index', 'Failed to delete {$lowercaseName}: ' . \$e->getMessage());
       }
    }
}
EOT;
	...
	protected function generateValidationRules(string $table): array
	{
		...
		foreach ($columns as $column) {
			...
			$rules[$column] = $this->generateRuleForColumn($column, $columnType);
		}
	 ...
	 */
	protected function generateRuleForColumn(string $column, string $columnType): string
	{
		...
		if (Str::endsWith($column, '_id')) {
			$rule[] = 'exists:'.Str::plural(Str::beforeLast($column, '_id')).',id';
		} elseif ($column === 'email') {
			$rule[] = 'email';
