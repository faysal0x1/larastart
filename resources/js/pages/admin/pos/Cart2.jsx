import { ChevronDown, Clock, CreditCard, DollarSign, FileText, List, Minus, Plus, Search, ShoppingCart, Trash2, X } from 'lucide-react';
...
const POSSystemCart = ({ product }) => {
    ...
    const addToCart = (product) => {
        ...
        if (product.variations && product.variations.length > 0) {
            ...
            if (existingItem) {
                setCart(cart.map((item) => (item.id === product.id && !item.variationId ? { ...item, quantity: item.quantity + 1 } : item)));
            } else {
                setCart([
                    ...cart,
                    {
    ...
    const updateQuantity = (productId, quantity, variationId = null) => {
        ...
        setCart(
            cart.map((item) =>
                item.id === productId && (variationId ? item.variationId === variationId : !item.variationId) ? { ...item, quantity } : item,
            ),
        );
    };
    ...
    return (
        <div className="flex h-screen flex-col bg-gray-100 pt-3">
            ...
            <div className="mx-14 flex flex-1 overflow-hidden">
                {/* Left panel - Cart */}
