...
export default function show() {
    const { subcategory } = usePage().props;

    if (!subcategory) {
        return (
            <AppLayout>
                <Head title="SubCategory Not Found" />
                <div className="py-12">
                    <div className="mx-auto max-w-7xl sm:px-6 lg:px-8">
                        <div className="text-center">
                            <h1 className="text-2xl font-bold text-gray-900">SubCategory Not Found</h1>
                            <p className="mt-2 text-gray-600">The requested subcategory could not be found.</p>
                        </div>
                    </div>
                </div>
            </AppLayout>
    ...
    return (
        <AppLayout>
            <Head title={`${subcategory.name} - SubCategory Details`} />
            <div className="py-12">
                <div className="mx-auto max-w-7xl sm:px-6 lg:px-8">
                    {/* Header */}
                    <div className="mb-8">
                        <div className="flex items-center justify-between">
                            ...
                            <div className="flex space-x-3">
                                <Link
                                    href={`/admin/sub-category/${subcategory.id}/edit`}
                                ...
                                <div className="px-4 py-5 sm:p-6">
                                    ...
                                    <dl className="grid grid-cols-1 gap-x-4 gap-y-6 sm:grid-cols-2">
                                        <div>
                                            ...
                                            <dd className="mt-1 text-sm text-gray-900">{subcategory.name}</dd>
                                        </div>
                                        <div>
                                            <dt className="text-sm font-medium text-gray-500">Slug</dt>
                                            <dd className="mt-1 text-sm text-gray-900">{subcategory.slug}</dd>
                                        </div>
                                        <div>
                                            <dt className="text-sm font-medium text-gray-500">Priority</dt>
                                            <dd className="mt-1 text-sm text-gray-900">{subcategory.priority || 'Not set'}</dd>
                                        </div>
                                        <div>
                                        ...
                                        <div>
                                            ...
                                            <dd className="mt-1 text-sm text-gray-900">{subcategory.level}</dd>
                                        </div>
                                        <div>
                                            <dt className="text-sm font-medium text-gray-500">Path</dt>
                                            <dd className="mt-1 text-sm text-gray-900 font-mono">{subcategory.path || 'N/A'}</dd>
                                        </div>
                            ...
                            {subcategory.parent && (
                                <div className="mt-8 bg-white shadow rounded-lg">
                                    <div className="px-4 py-5 sm:p-6">
                                        ...
                                        <div className="flex items-center space-x-4">
                                            {subcategory.parent.image_url && (
                                                <img
                                                    className="w-16 h-16 rounded-lg object-cover"
                                                    src={subcategory.parent.image_url}
                                                    alt={subcategory.parent.name}
                                                />
                                            )}
                                            <div>
                                                <h4 className="text-lg font-medium text-gray-900">{subcategory.parent.name}</h4>
                                                <p className="text-sm text-gray-500">{subcategory.parent.slug}</p>
                                                <p className="text-sm text-gray-600">{subcategory.parent.description}</p>
                                            </div>
                                        </div>
                                    </div>
                                </div>
                                ...
                                <div className="mt-8 bg-white shadow rounded-lg">
                                    <div className="px-4 py-5 sm:p-6">
                                        <h3 className="text-lg font-medium text-gray-900 mb-4">
                                            Child Categories ({subcategory.children.length})
                                        </h3>
                                        ...
                                        <div className="overflow-hidden">
                                            <table className="min-w-full divide-y divide-gray-200">
                                                ...
                                                <tbody className="bg-white divide-y divide-gray-200">
                                                    {subcategory.products.slice(0, 10).map((product) => (
                                                        <tr key={product.id}>
                                            ...
                                            {subcategory.products.length > 10 && (
                                                <div className="px-6 py-3 text-sm text-gray-500 text-center">
                                                    And {subcategory.products.length - 10} more products...
                                                </div>
                                            )}
                                        </div>
                                    </div>
                                ...
                                <div className="px-4 py-5 sm:p-6">
                                    ...
                                    <dl className="space-y-4">
                                        ...
                                        <div>
                                            ...
                                            <dd className="mt-1">
                                                <span className={`inline-flex px-2 py-1 text-xs font-semibold rounded-full ${subcategory.is_featured
                                        ...
                                        <div>
                                            ...
                                            <dd className="mt-1">
                                                <span className={`inline-flex px-2 py-1 text-xs font-semibold rounded-full ${subcategory.is_active
                                                    ? 'bg-green-100 text-green-800'
                                                    : 'bg-red-100 text-red-800'
                                                    }`}>
                                        ...
                                        <dl className="space-y-4">
                                            {subcategory.meta_title && (
                                                <div>
                                                    ...
                                                    <dd className="mt-1 text-sm text-gray-900">{subcategory.meta_title}</dd>
                                                </div>
                                            )}
                                            {subcategory.meta_description && (
                                                <div>
                                                    <dt className="text-sm font-medium text-gray-500">Meta Description</dt>
                                                    <dd className="mt-1 text-sm text-gray-900">{subcategory.meta_description}</dd>
                                                </div>
                                            )}
                                        </dl>
                                    </div>
