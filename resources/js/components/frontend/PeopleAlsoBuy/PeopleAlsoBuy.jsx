...
const PeopleAlsoBuy = ({ cartProductIds = [] }) => {
    const { props } = usePage();
    ...
    const fetchPopularProducts = useCallback(() => {
        console.log('🟡 [People Also Buy] Fetching popular products as last resort');
    ...
    const fetchRecommendationsWithProducts = useCallback(async (algorithmToUse, productIdsToUse, userIdToUse, sessionIdToUse, isDefault = false) => {
        try {
            setLoading(true);
            if (isDefault) {
                setIsUsingDefault(true);
                console.log('🟡 [People Also Buy] Using default recommendations');
            } else {
            ...
            if (productIdsToUse && productIdsToUse.length > 0) {
                // For frequently_bought_together, pass product_ids as array
                productIdsToUse.forEach(id => {
            ...
            if (!isDefault) {
                // Try default recommendations (most purchased)
                await fetchRecommendationsWithProducts('most_purchased_v1', null, userIdToUse, sessionIdToUse, true);
            } else {
    ...
    useEffect(() => {
        if (cartProductIds && cartProductIds.length > 0) {
            // Use frequently bought together algorithm with cart product IDs
            const sessionId = getSessionId();
            fetchRecommendationsWithProducts('fbt_v1', cartProductIds, user?.id || null, sessionId, false);
        } else {
            // If cart is empty, use most purchased
            const sessionId = getSessionId();
            fetchRecommendationsWithProducts('most_purchased_v1', null, user?.id || null, sessionId, true);
        }
    }, [cartProductIds, user?.id, fetchRecommendationsWithProducts]);
    ...
    // Custom swiper settings - show 3 products at a time
    const customSwiperSettings = {
        ...
        loop: transformedProducts.length > 3,
        breakpoints: {
