...
const ModalQuickview = ({ isOpenQuickView, onCloseQuickView, product }) => {
  ...
  // This effect will run when isCartModalOpen changes to true
  useEffect(() => {
    if (isCartModalOpen) {
      // Now it's safe to close the quick view modal
      const timer = setTimeout(() => {
        onCloseQuickView(); // This closes the quick view modal
      }, 50); // Small delay to ensure CartModal renders

      return () => clearTimeout(timer);
    }
  }, [isCartModalOpen, onCloseQuickView]);
  ...
  const handleAddToCart = () => {
    ...
    const productData = {
      id: product.id,
      title: product.title,
      price: isOnSale ? product.discountedPrice : product.price,
  ...
  return (
    <>
          ...
          <div className="flex flex-col md:flex-row gap-6">
            ...
            <div className="w-full md:w-1/2 px-4">
              ...
              <div className="flex gap-2 overflow-x-auto pb-2">
                {/* First show all images for the selected color */}
                ...
                {/* Then show all other color images */}
                {Object.entries(product.images).map(([color, images]) => {
                  if (color === selectedColor) return null; // Skip already shown images

                  return images.map((image, index) => (
                    <div
                      key={`${color}-${index}`}
                      className="aspect-square w-20 rounded-lg overflow-hidden cursor-pointer opacity-70 hover:opacity-100"
                      onClick={() => {
            ...
            <div className="w-full md:w-1/2 px-4">
              ...
              <div className="mb-4">
                ...
                <div className="flex items-center gap-4">
                  ...
                  <button
                    className="border border-black w-full py-2 px-4 flex items-center justify-center gap-2 hover:bg-black hover:text-white transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
              ...
              {/* Buy Now */}
              <button
                className="w-full py-2 bg-black text-white rounded-lg mb-4 hover:bg-gray-800 transition-colors flex items-center justify-center gap-2 disabled:opacity-50 disabled:cursor-not-allowed"
      ...
      {/* <CartModal
        ...
        onClosCart={handleCartModalClose}
        // product={cartItems}
      ...
      <CartModal
        isOpenCart={isCartModalOpen}
        onClosCart={() => setIsCartModalOpen(false)}
        onUpdateCart={setCartItems}
      />
    </>
  );
