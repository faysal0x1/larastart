...
const BestSellers = () => {
  ...
  return (
    <section className="container mx-auto px-4 py-12">
            ...
            <SwiperSlide key={product.id}>
              ...
              >
                ...
                <div className="mt-4 relative h-20">
                  ...
                  <div className={`absolute top-0 left-0 flex gap-2 transition-opacity ${hoveredProduct === product.id ? 'opacity-100' : 'opacity-0'
                    }`}>
                    {product.colors.map(color => (
                      <button
                        key={color}
                        className={`w-5 h-5 rounded-full border-2 cursor-pointer transition-transform hover:scale-110 ${selectedColor === color ? 'border-black' : 'border-transparent'
                          }`}
      ...
      {quickViewModal.open && (
        <ModalQuickview
          isOpenQuickView={quickViewModal.open}
