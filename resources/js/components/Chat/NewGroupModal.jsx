...
export default function NewGroupModal({ show, onClose, users }) {
    ...
    const toggleUser = (userId) => {
        setData('user_ids', data.user_ids.includes(userId) ? data.user_ids.filter((id) => id !== userId) : [...data.user_ids, userId]);
    };

    const submit = (e) => {
        e.preventDefault();
        post(route('chat.group.create'), {
            onSuccess: () => onClose(),
        });
    };
